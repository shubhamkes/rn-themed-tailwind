...
import base from './temp-styles/styles-base.json';
import themes from './temp-styles/styles-themes.json';
...
export const ThemeContextProvider = ({ children, value = 'light' }) => {
    ...
    useEffect(() => {
        ...
        AsyncStorage.getItem(THEME_ASYNCSTORAGE).then(lastTheme => {
            ...
            if (lastTheme && lastTheme != state.theme) {
    ...
    const reducer = (state, action) => {
        ...
        if (state.theme != action.name || !Object.keys(state).length) {
            AsyncStorage.setItem(THEME_ASYNCSTORAGE, action.name);
